...
public:
	AutoEmote() : Module("PacketEmote", "Emotes automatically using packets, this wont be visible to you tho, only to other players", Category::MISC) {
		registerSetting(new SliderSetting<int>("Delay", "NULL", &emoteDelay, 10, 0, 50));
	}
	virtual void onNormalTick(LocalPlayer* localPlayer) override {
		static float lastScan = 0;
		if (!TimerUtil::hasReached<ticks>(&lastScan, emoteDelay)) return;
		if (index >= emoteUUIDs.size()) index = 0;
		std::shared_ptr<Packet> sharedPkt = MinecraftPackets::createPacket(PacketID::Emote);
		EmotePacket* emotePacket = (EmotePacket*)sharedPkt.get();
		emotePacket->mRuntimeId = localPlayer->getRuntimeIDComponent()->runtimeId.id;
		emotePacket->mXuid = emotePacket->mXuid;
		emotePacket->mPieceId = emoteUUIDs[index];
		localPlayer->level->getPacketSender()->send(emotePacket);
		index++;
	}
};