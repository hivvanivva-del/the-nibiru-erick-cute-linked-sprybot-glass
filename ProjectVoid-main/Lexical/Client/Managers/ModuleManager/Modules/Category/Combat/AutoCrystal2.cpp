...
AutoCrystal2::AutoCrystal2() : Module("AutoCrystalX", "Place and break crystals", Category::COMBAT) {
	...
	registerSetting(new BoolSetting("Phase Place", "In IGNPVP, when an entity is phased vertically or phased far enough horizontally, explosions can go through blocks to deal full damage to him\nthis will allow the AutoCrystal2 to place on places that would normally deal zero damage", &phasePlace, false));
	registerSetting(new BoolSetting("Multitask", "If this is on then the client will continue AutoCrystal2ling the enemy even if you are doing something(like eating, using a bow)", &multiTask, false));
...
bool AutoCrystal2::sortByDamage(CrystalData d1, CrystalData d2) {
	return d1.targetDamage > d2.targetDamage;
}

bool AutoCrystal2::isPlaceValid(const BlockPos& blockPos, Actor* actor) {
...
float AutoCrystal2::getExposure(const Vec3<float>& blockPos, Actor* actor) const {
	if (phasePlace) {
		const Vec3<float>& actorPos = actor->getPos().floor().sub(0.f, 1.f, 0.f);
...
bool AutoCrystal2::isInList(const BlockPos& blockPos) {
...
void AutoCrystal2::generatePlacement(LocalPlayer* localPlayer, Actor* actor) {
	...
	std::sort(placeList.begin(), placeList.end(), AutoCrystal2::sortByDamage);
}

bool AutoCrystal2::isTerrainValid(const BlockPos& blockPos, Actor* actor) {
...
void AutoCrystal2::getTerrainList(Actor* actor) {
	...
	std::sort(terrainList.begin(), terrainList.end(), AutoCrystal2::sortByDamage);
}

void AutoCrystal2::placeObsidian(GameMode* gm) {
...
void AutoCrystal2::onTerrain(LocalPlayer* localPlayer, Actor* actor) {
	if (!placeList.empty()) {
		terrainList.clear();
		return;
	}
...
void AutoCrystal2::getCrystalList(LocalPlayer* localPlayer, Actor* actor) {
	for (Actor* entity : entityList) {
	...
	std::sort(breakList.begin(), breakList.end(), AutoCrystal2::sortByDamage);
}

void AutoCrystal2::attackCrystal(GameMode* gm, LocalPlayer* localPlayer) {
...
void AutoCrystal2::placeCrystal(GameMode* gm, LocalPlayer* localPlayer) {
	if (placeList.empty()) return;
...
void AutoCrystal2::onNormalTick(LocalPlayer* localPlayer) {
	terrainList.clear();
	placeList.clear();
	breakList.clear();
	entityList.clear();
...
void AutoCrystal2::onSendPacket(Packet* packet, bool& cancel) {
	if (placeList.empty()) return;
	if (!rotate) return;
...
void AutoCrystal2::onMCRender(MinecraftUIRenderContext* renderCtx) { // this is so retarded
